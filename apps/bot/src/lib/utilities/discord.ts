import { BlankSpace, EmbedColors, GuildEmoteSlots, GuildSoundboardSlots, GuildStickerSlots, ShinBotEmoji } from '#lib/utilities/constants';
...
export async function getUserInfo(interaction: AnyInteraction, userId: string): Promise<EmbedBuilder> {
	...
	if (member) {
		return embed
			...
			.setFooter({ text: `Present in server: ${ShinBotEmoji.GreenCheck}` });
	}

	const banned = await interaction.guild?.bans
		.fetch(userId)
		.then((ban) => `${ShinBotEmoji.GreenCheck} User is banned\nReason: ${ban.reason}`)
		.catch(() => `${ShinBotEmoji.RedX} User is not banned`);
	...
	return embed
		...
		.setDescription(`${userMention(user.id)} | ${bot}`)
		.setFooter({ text: `Present in server: ${ShinBotEmoji.RedX}` });
}
